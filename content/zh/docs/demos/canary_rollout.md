...
3. 为客户端的访问，创建根服务 `httpbin`，服务选择器使用 `app: httpbin`。
...
4. 部署 `httpbin` 服务的 `v1` 版本。该服务 `httpbin-v1` 使用选择器 `app: httpbin, version: v1`， 同时，该 deployment `httpbin-v1` 有标签 `app: httpbin, version: v1` ，其匹配了根服务 `httpbin` 和服务 `httpbin-v1` 的选择器。
...
7. 确认发往根服务 FQDN `httpbin.httpbin.svc.cluster.local` 的所有流量都被路由到 `httpbin-v1` pod 中。可以通过检查 HTTP 响应头, 确认请求成功和 pod对应的展示相关信息到 `httpbin-v1` 。
...
8. 准备金丝雀发布 - 部署 `httpbin` 的 `v2` 版本。该服务 `httpbin-v2` 使用选择器 `app: httpbin, version: v2`，同时，该 deployment 的标签 `app: httpbin, version: v2` 匹配根服务 `httpbin` 和服务 `httpbin-v2` 的选择器。
...
9. 执行金丝雀发布 - 通过更新 SMI TrafficSplit 资源， 将发往 FQDN `httpbin.httpbin.svc.cluster.local` 的流量拆分到 `httpbin-v1` 和 `httpbin-v2` 服务，分别对应 `httpbin` 的 `v1` 和 `v2` 版本。。
